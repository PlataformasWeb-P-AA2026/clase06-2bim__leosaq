...
token = "4c385f855b30eaf7783e8ef52cb61fe7a903ca7b"
...
def los_estudiantes():
    """ """
    r = requests.get(
        "http://localhost:8000/api/estudiantes/", auth=("antadmin", "admin12345")
    )
    print("---------------------")
    print(r.content)
    print("---------------------")
...
def los_telefonos():
    """ """
    r = requests.get("http://localhost:8000/api/numerosts/", auth=("rene", "1"))
    datos = json.loads(r.content)["results"]
    numero = json.loads(r.content)["count"]
    return render_template("lostelefonos.html", datos=datos, numero=numero)


@app.route("/los/telefonos/dos")
def los_telefonos_dos():
    """ """
    r = requests.get("http://localhost:8000/api/numerosts/", headers=headers)
    datos = json.loads(r.content)["results"]
    numero = json.loads(r.content)["count"]
    datos2 = []
    for d in datos:
    ...
    return render_template("lostelefonosdos.html", datos=datos2, numero=numero)
...
def agregar_estudiante():
    ...
    if request.method == "POST":
        ...
        flash(
            f"Estudiante '{nuevo_estudiante['nombre']} {nuevo_estudiante['apellido']}' creado exitosamente!",
            "success",
        )
        return redirect(
            url_for("los_estudiantes")
...
def crear_numero_telefonico():
    ...
    r_estudiantes = requests.get(
        "http://localhost:8000/api/estudiantes/", headers=headers
    )
    ...
    if request.method == "POST":
        ...
        r = requests.post(
            "http://localhost:8000/api/numerosts/",
        ...
        flash(
            f"Número '{nuevo_numero['telefono']}' creado exitosamente para el estudiante!",
            "success",
        )
        return redirect(
            url_for("los_telefonos")
        )  # Redirigir a la lista principal o a una de números

    return render_template(
        "crear_numero_telefonico.html",
...
def crear_direccion():
    ...
    if request.method == "POST":
        ...
        if r.status_code == 201:
            ...
            flash(
                f"Dirección '{nueva_direccion['descripcion']}' creada exitosamente!",
                "success",
            )
            return redirect(url_for("los_estudiantes"))
        else:
            flash("Error al crear la dirección. Revisa la consola.", "danger")

    return render_template("crear_direccion.html", estudiantes=estudiantes_disponibles)
